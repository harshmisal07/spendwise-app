...
export default function HistoryScreen() {
  ...
  const activeCategories = useMemo(() => {
    ...
    return [
      { id: "all", name: "All", icon: "apps", color: "#6C5CE7" },
    ...
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      <View style={[styles.header, { paddingTop: topPad + 8 }]}>
        ...
        <View style={styles.headerTop}>
          ...
          <TouchableOpacity onPress={() => setShowSortMenu((v) => !v)} style={[styles.sortBtn, { backgroundColor: colors.card, borderColor: colors.border }]}>
        ...
        {showSortMenu && (
          <View style={[styles.sortMenu, { backgroundColor: colors.card, borderColor: colors.border }]}>
            {SORT_OPTIONS.map((o) => (
              <TouchableOpacity key={o.key} onPress={() => { setSort(o.key); setShowSortMenu(false); Haptics.selectionAsync(); }} style={[styles.sortMenuItem, { borderBottomColor: colors.border }]}>
                <Text style={[styles.sortMenuText, { color: sort === o.key ? colors.primary : colors.foreground, fontFamily: sort === o.key ? "Inter_600SemiBold" : "Inter_400Regular" }]}>{o.label}</Text>
        ...
        <View style={styles.pills}>
          {(["all", "income", "expense"] as Filter[]).map((f) => (
            <TouchableOpacity key={f} onPress={() => { setFilter(f); Haptics.selectionAsync(); }} style={[styles.pill, { backgroundColor: filter === f ? "#6C5CE7" : colors.muted }]}>
        ...
        {activeCategories.length > 2 && (
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.catScroll} contentContainerStyle={styles.catScrollContent}>
            {activeCategories.map((cat) => {
              ...
              return (
                <TouchableOpacity key={cat.id} onPress={() => { setCategoryFilter(cat.id); Haptics.selectionAsync(); }}
                  style={[styles.catChip, { backgroundColor: active ? cat.color + "20" : colors.card, borderColor: active ? cat.color : colors.border, borderWidth: active ? 1.5 : 1 }]}>
                  <Ionicons name={cat.icon as any} size={12} color={active ? cat.color : colors.mutedForeground} />
                  <Text style={[styles.catChipText, { color: active ? cat.color : colors.mutedForeground, fontFamily: active ? "Inter_600SemiBold" : "Inter_400Regular" }]}>{cat.name}</Text>
                </TouchableOpacity>
              );
            })}
          </ScrollView>
        ...
        {/* Mini summary for filtered results */}
        {(filter !== "all" || categoryFilter !== "all" || search) && filtered.length > 0 && (
          <View style={[styles.filterSummary, { backgroundColor: colors.card, borderColor: colors.border }]}>
            {filter !== "expense" && <Text style={[styles.filterSummaryText, { color: "#00B894" }]}>↑ {format(filteredIncome)}</Text>}
            {filter !== "income"  && <Text style={[styles.filterSummaryText, { color: "#FF6B6B"  }]}>↓ {format(filteredExpenses)}</Text>}
            <Text style={[styles.filterSummaryText, { color: colors.mutedForeground }]}>Net: {format(filteredIncome - filteredExpenses)}</Text>
          </View>
        )}
      </View>
        ...
        ListEmptyComponent={
          <View style={styles.empty}>
            ...
            <Text style={[styles.emptyText, { color: colors.mutedForeground }]}>
              {search || filter !== "all" || categoryFilter !== "all" ? "Try different search terms or filters" : "Add your first transaction from the dashboard"}
            </Text>
          </View>
        }
...
const styles = StyleSheet.create({
  ...
  sortBtn: { flexDirection: "row", alignItems: "center", gap: 6, borderRadius: 10, paddingHorizontal: 10, paddingVertical: 7, borderWidth: 1, maxWidth: 160 },
  sortBtnText: { fontSize: 12, fontFamily: "Inter_500Medium" },
  sortMenu: { borderRadius: 14, borderWidth: 1, marginBottom: 12, overflow: "hidden", shadowColor: "#000", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.1, shadowRadius: 8, elevation: 4 },
  sortMenuItem: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: 16, paddingVertical: 13, borderBottomWidth: 1 },
  sortMenuText: { fontSize: 14 },
  searchWrap: { flexDirection: "row", alignItems: "center", gap: 8, borderRadius: 12, borderWidth: 1, paddingHorizontal: 12, height: 44, marginBottom: 10 },
  ...
  catChip: { flexDirection: "row", alignItems: "center", gap: 5, borderRadius: 20, paddingHorizontal: 11, paddingVertical: 6 },
  catChipText: { fontSize: 12 },
  filterSummary: { flexDirection: "row", gap: 14, borderRadius: 10, borderWidth: 1, paddingHorizontal: 14, paddingVertical: 8, marginBottom: 6 },
